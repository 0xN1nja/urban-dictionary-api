...
def get_word():
    ...
    try:
        ...
        try:
            ...
            for i,j in enumerate(soup.find_all("div",class_="meaning")):
                more_meanings[i]["mug_back_image"]=f"https://urbandictionary.store/render/preview/mug/back.webp?bg=FFF200&fg=000000&fill=FFFFFF&logo-variant=dark&meaning={j.text}&word={query}".replace(" ","%20").replace("'","").replace('"',"")
        ...
        # Mug Front Image
        mug_front_image=f"https://urbandictionary.store/render/preview/mug/front.webp?bg=FFF200&fg=000000&fill=FFFFFF&logo-variant=dark&word={query}".replace(" ","%20").replace("'","").replace('"',"")
        # Mug Back Image
        mug_back_image=f"https://urbandictionary.store/render/preview/mug/back.webp?bg=FFF200&fg=000000&fill=FFFFFF&logo-variant=dark&meaning={meaning}&word={query}".replace(" ","%20").replace("'","").replace('"',"")
        json={
            "word":query,
            "meaning":meaning,
            "example":example,
